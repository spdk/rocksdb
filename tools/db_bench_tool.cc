...
DEFINE_int32(spdk_shared_mem, -1, "Shared memory region id from stub application");
...
// definition of spdk_opt structure for initializing spdk_env
struct spdk_opts{
	uint64_t cache_size;
	int32_t shm_id;
};
...
int db_bench_tool(int argc, char** argv) {
  ...
  if (!FLAGS_spdk.empty()) {
    struct spdk_opts opts;
    opts.cache_size = FLAGS_spdk_cache_size;
    opts.shm_id = FLAGS_spdk_shared_mem;
    FLAGS_env = rocksdb::NewSpdkEnv(rocksdb::Env::Default(), FLAGS_db, FLAGS_spdk, FLAGS_spdk_bdev, &opts, sizeof(struct spdk_opts));
    if (FLAGS_env == NULL) {
