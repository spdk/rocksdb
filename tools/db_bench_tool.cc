...
DEFINE_int32(spdk_shared_mem, 0, "Shared memory region id from stub application");
...
int db_bench_tool(int argc, char** argv) {
  ...
  if (!FLAGS_spdk.empty()) {
    FLAGS_env = rocksdb::NewSpdkEnv(rocksdb::Env::Default(), FLAGS_db, FLAGS_spdk, FLAGS_spdk_bdev, FLAGS_spdk_cache_size, FLAGS_spdk_shared_mem);
    if (FLAGS_env == NULL) {
